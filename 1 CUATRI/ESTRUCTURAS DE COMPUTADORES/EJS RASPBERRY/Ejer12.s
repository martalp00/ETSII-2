...
@comparamos el pulsador GPIO2
		ands r1, r4, r5
		bne sent @si es el pulsador 2 enciende los leds
@comparamos el GPIO3
		ands r1, r4, r3 @ si es el pulsador 3 el parpadeo va normal y seguimos con la secuencia
		bne otrosent
...
@Apago todos LEDs 10987654321098765432109876543210
@PROBAR CON UN 1 EN EL PIN 4 PARA APAGAR EL ALTAVOZ A VER SI FUNCIONA
apagao:
		ldr r2,      =0b00001000010000100000111000010000
		str r2, [r0, #GPCLR0]
		ldr r4, [r0, #GPEDS0]
		ldr r0, =GPBASE
...
/*	guia bits 		   xx999888777666555444333222111000 */
		...
		ldr 	r1, =0b00000000001000000000000001000000
		str 	r1, [r0, #GPFSEL2]
		ldr 	r6, =0 						@registro que me muestra en qu» sentido van los leds,con 0 hacia la derecha y con 1 hacia la izquierda
...
@ metemos los dos pulsadores en distintos registros
		mov 	r5, #0b00000000000000000000000000000100 @pulsador 1GPIO2
		mov 	r3, #0b00000000000000000000000000001000 @pulsador 2GPIO3
		ldr 	r4, [r0, #GPEDS0]
@comparamos el pulsador GPIO2
		cmp 	r4, r5
...
@Leo cuenta y luego elemento correspondiente en secuen
		ldr 	r10, [r9] @leo cuenta
		ldr 	r9, [r9, +r10, LSL #3] @desplazo la cuenta (multiplico por 8) y le sumo la direccion de secuen
...
@Reseteo estado interrupci€n de C3
		...
		mov 	r10, #0b1000
		str 	r10, [r8, #STCS]
@Programo retardo seg?n valor leÃdo en array
		ldr 	r10, [r8, #STCLO]
		add 	r10, r9
		str 	r10, [r8, #STC3]
		subs 	pc, lr, #4		