...
public class Huffman {
    ...
    public static Dictionary<Character, Integer> weights(String s)
    {
        ...
        if(dict.size() == 1)
        {
            throw new huffman.HuffmanException("El mensaje tiene que tener  mínimo dos 2 caracteres distintos");
        }
        return dict;
    }

    // Exercise 2.a 
    public static PriorityQueue<huffman.WLeafTree<Character>> huffmanLeaves(String s)
    {
    	PriorityQueue<huffman.WLeafTree<Character>> pq = new BinaryHeapPriorityQueue<>();
    	for(Tuple2<Character, Integer> t : weights(s).keysValues())
        {
            pq.enqueue(new huffman.WLeafTree<Character>(t._1(), t._2()));
        }
        return pq;
    }

    // Exercise 2.b  
    public static huffman.WLeafTree<Character> huffmanTree(String s)
    {
    	// s debe tener dos caracteres distintos. Si no, excepcion
        PriorityQueue<huffman.WLeafTree<Character>> pq = huffmanLeaves(s);
        huffman.WLeafTree<Character> wlt, wlt1, wlt2;
        wlt1 = pq.first();
        pq.dequeue();
        wlt2 = pq.first();
        pq.dequeue();

        wlt = new huffman.WLeafTree<Character>(wlt1, wlt2);
        while (!pq.isEmpty())
        {
            wlt = new huffman.WLeafTree<Character>(pq.first(), wlt);
            pq.dequeue();
        }
        return wlt;
    }
    ...
    // Exercise 3.c  
    public static Dictionary<Character, List<Integer>> huffmanCode(huffman.WLeafTree<Character> ht)
    {
        if(ht.isLeaf())
        {
            Dictionary<Character, List<Integer>> dict = new AVLDictionary<>();
    ...
    // Exercise 5 
    public static String decode(List<Integer> bits, huffman.WLeafTree<Character> ht)
    {
        huffman.WLeafTree<Character> aux = ht;
        String str ="";
        for(int i:bits)
        {
            if(i==0)
