 ...
 */public class LinearProbingHashTable<K,V> implements HashTable<K,V>{
	...
	private int searchIdx(K key)
	{
		...
		while((keys[indice] != null) && (!keys[indice].equals(key)))
		{
			indice = (indice + 1) % keys.length;
		}
		return indice;
	}
	...
	public void insert(K key, V value)
	{
		...
		if(search(key) == null)
		{
			keys[searchIdx(key)] = key;
			size++;
		}
		values[searchIdx(key)] = value;
	}
	...
	public void delete(K key)
	{
		...
		int ind2 = searchIdx(key);

		if(isElem(key))
		{
			aNull(keys, values, ind2);
	...
	private void aNull(K key[], V value[], int param)
	{
		value[param] = null;
		key[param] = null;
	}
